...
const DashboardHero = () => {
  ...
  const availableBalance = formatNumber.format(seller?.availableBalance || 0);
  ...
  orders &&
    orders.forEach((item) => {
      row.push({
        ...
        total: formatNumber.format(item.totalPrice) + "đ",
        status: item.status,
      });
    });
  return (
      ...
      <div className="w-full block 800px:flex items-center justify-between">
        <div className="w-full mb-4 800px:w-[30%] min-h-[20vh] bg-white shadow rounded px-2 py-5">
          <div className="flex items-center">
            ...
            >
              Số dư tài khoản
              <span className="text-[16px]">(với 10% phí dịch vụ)</span>
            </h3>
          </div>
